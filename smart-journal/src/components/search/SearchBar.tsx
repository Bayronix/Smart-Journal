      ...
      <AnimatePresence>
        {focused && (
          ...
          >
            ...
            {results.length > 0 && (
              <div>
                <p className="px-3 pt-3 pb-1 text-[10px] text-zinc-600 uppercase tracking-wider font-semibold">Results</p>
                {results.map((r) => (
                  <Link
                    key={r.entry.id}
            ...
            {!query && (
              <div>
                <p className="px-3 pt-3 pb-1 text-[10px] text-zinc-600 uppercase tracking-wider font-semibold">Try asking</p>
                {SUGGESTIONS.map((s) => (
                  <button
                    key={s}
            ...
            {query && results.length === 0 && !loading && (
              <p className="px-3 py-4 text-sm text-zinc-600 text-center">No matches found.</p>
            )}
          </motion.div>
        )}
      </AnimatePresence>
